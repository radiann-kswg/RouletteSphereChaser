...
- ✅ ボールUV再設計済み（2026-08-22）: **本体UVは「前後2円ディスク」方式**（Userの参考モデル`UVSphere_Test`準拠）。
  - キャンバスは**2:1**（例2048×1024）。**左円=前半球**（Unity +Z 正面、方位等距離図法・円中心=顔の中心）、**右円=後半球**（左右鏡像=後ろから見た絵をそのまま描ける）。円は画像上で真円（UV空間ではu半径0.24/v半径0.48）
  - 番号パッチ（±X・アトラスUV・submesh1）は従来通り維持。FBX鏡像対策のu反転は両UVに適用済み
  - アーティスト用テンプレート: `Docs/BallUV_Template.png`。テスト用テクスチャ: `Assets/Textures/RefBallTex.png`（User作・猫キャラ）。UV変更時は必ずUnity側で正面レンダリングして鏡像チェック（前髪の流れ・ポニーテール位置で判定）
