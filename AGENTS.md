...
- ✅ **フェーズ6.6 完了（2026-08-24）: メッシュ健全性の総ざらい**（別環境の `LiftGuide` 修正コミットの続き）。詳細は `Docs/DESIGN-v2.md` 6章「フェーズ6.6」。要点:
  - `Assets/Models/*.fbx` 全54ファイルをスイープ →**使用中メッシュの縮退面・ゼロ法線をすべて0**にした（罠49の申し送りを解消）。
  - 最悪だった `TowerA_Distributor` は 4599→1372面に痩せたが、**符号付き体積 Δ4.2e-08・バウンズ完全一致・8方位の床レイキャスト完全一致**＝形は不変。
  - `TowerG_NumaKuruun_S` のゼロ法線は縮退面ではなく**「厚み0.0046の薄壁＋スムーズシェード」で頂点法線が打ち消し合う**別原因だった（罠49に追記）。
  - **新規検出**: 沼クルーン L/M/S は3皿ともUnityインポータに**20枚前後のポリゴンを self-intersecting で破棄されている**＝コライダに穴。作り直し案件（DESIGN-v2 6章「積み残し」）。
  - `LiftGuide` はメッシュとしては健全（縮退0・ゼロ法線0・非多様体0・頂点法線の破綻0）だが、**円環状N-gonのチューブ端キャップ8枚**が同じく破棄されている。チューブ内側なので見た目には出ない。三角化は円環を潰すので不可（罠50）。
  - 併せて **タワーE給球パンを 1.40×0.52 → 2.05×0.68 へ拡幅**（C排出が遅いとパン東端と盤の隙間からコースアウトしていた・User報告）、**C戻りトラフを長手0.8倍に短縮**（5.77→4.61m・西端の排出点は不変）。
  - 36球ソークで回帰なしを確認（564秒・216巡・平均51.2/巡・場外0）。
- ✅ **フェーズ6.7 完了（2026-08-24）: タワー構造の最終調整（User指示）**。詳細は `Docs/DESIGN-v2.md` 6章「フェーズ6.7」。要点:
  - **高得点チャレンジ（D上段・Eチャレンジ盤）は加点せず「次の通常抽選の倍率」を与える**方式へ:
    `ScoreZone.grantMultiplier` ＋ `LotteryBall.nextMultiplier`（通常段の加点で消費・CommitLapで失効）。ビルダーは `MultMark()`。
  - **E二段化**: チャレンジ盤(0.10, 3.85)を新設し JPチューブ5段→3段。チャレンジ盤の全排出が給球パン→通常盤へ。パンはテーパー形へ作り直し。
...
- ✅ **フェーズ9-2 完了（2026-08-24）: コンセプトの明文化・中性色への後退・透過シェルで死角一掃**。
  設計正本は `Docs/DESIGN-materials.md`（0章＝コンセプト／2章＝パレット／2.5章＝透過シェル）。要点:
  - **プロダクトのコンセプトを1章に明文化**（想定ユーザー2種・「静かであること」「中が見えること」が上位ルール）。
    以後の外装判断はこれに従う。**見栄えが良くてもコンセプトと矛盾する案は採らない。**
  - **配色を中性色へ後退**（User指摘「アクセントが激しすぎる」）: 金・水色のレーン→**白**、赤・橙の台→**銀**、
    得点ゲートのシアン発光 ×1.6 → **やわらかい白 ×0.70**。**色味を持つのは抽選盤（紺）だけ**にし、
    役割は**明度3段（白レーン／銀の台／ガンメタ搬送）と金属感**で分ける。
  - **`CameraCoverage` に遮蔽者の集計（`blockers`）を追加**。これで初めて「どのメッシュを直せばいいか」が数字で出た。
  - **透過アクリル化で死角を一掃**（罠52の追記を参照。窓抜き・低背化は採らなかった）。実測:

    | カメラ | 前 | 後 |
    |---|---|---|
    | `Cam_A_GrandRoulette` | 0.844 | **0.137** |
    | `Cam_C_Zigzag_N / _S` | 0.776 / 0.737 | **0.108 / 0.092** |
    | `Cam_G_Numa_W / _E` | 0.720 / 0.646 | **0.156 / 0.181** |
    | `Cam_H_Garapon` | 0.708 | **0.243** |
    | `Cam_A_Overview` | 0.766 | **0.329** |
    | `Cam_B_Pachinko_E / _W` | 0.879 / 0.870 | **0.511 / 0.444** |

    **死角70%超の機構はゼロになった**（最悪でも `Cam_D_Kuruun_W` 0.559）。
  - **36球ソーク300秒で回帰なし**: 118巡・6,387点・コースアウト0・停止0・迷子0（`Docs/soak_seethrough.json`）。
