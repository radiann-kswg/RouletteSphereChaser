...
57. **`lib.reload()` 直後の `ob.bound_box` は古いキャッシュを返すことがある**（2026-08-30実測）。原本.blendのメッシュ形状を変えて
...
- ⚠️ 未解決1: **初動波の東回廊渋滞**（スポーン直後の集団が S字壁沿い x9.3-9.6 で一時滞留・約15%のスループット低下、118→98巡）。
  バンクフィレット案は**コーナーに新たな停留を作るため撤回済み**（乗り上げ斜面が横入り球を往復静止させる）。フェーズ7の負荷検証で
  支柱群（H三脚・Bフィン・E/Cレッグ）の流路影響と合わせて再調整すること。
- ⚠️ 未解決2: **B/Gの給球バランス**。Gトレイ（東壁天端7.70＝v≈4.5でスプリット）導入後、チェーン外向きスパウトの通過率が低く
  （実測: 約1球/2分/レーン）、B・G・D_W・Hの到達が薄い。スパウト延伸(r1.13)後の出口配分・トレイ東壁高さ・レーン発射速度を
  フェーズ7でC/P実測と共に再設計する。トレイのスロット落下帯は x5.35-6.15（クレーター直落ち回避済み）。
