...
19. **FBXの軸マッピングは2種類ある（実測）**: スパイラル等の機構系は素通し（Blender座標=Unity座標）、無回転単一メッシュのベース系はX軸ミラー。ベース系はZ対称に作りY180°回転で相殺（`InstantiateFbx`）。新規FBXは必ずインスタンス後にレイキャスト/バウンズで実測確認。
...
- **次の作業: タワーA②以降の設計見直し**。4連スパイラルの排出（中央シャフト4本・y9.55）を下段機構でどう受けるか `Docs/DESIGN-v2.md` の階層表を改訂してから着工。分配の偏り微調整は撹拌腕の羽根数/速度と `releaseJitter` が調整ノブ。
