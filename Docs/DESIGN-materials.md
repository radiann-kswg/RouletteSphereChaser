...
| 役割 | 模様 | 強さ |
|---|---|---|
| CABINET | パンチングメタル（丸穴 pitch 40px / r 0.30） | 0.18 |
| CHROME / LANE | ヘアライン（u方向の条線） | 0.06 / 0.03 |
| JACKPOT | ローレット（45°交差溝 pitch 24px） | 0.08 |
| DECK | 36分割の扇形トーン差＋境界線（u=方位） | 0.05 |
| ROTOR / ROTOR_HI | 矢羽根（回転方向）×ヘアライン | 0.05 / 0.03 |
| SCORE | 額縁（縁に向かって落ちる。中央はTMPラベルの背景） | 0.10 |
| SHOWPIECE | すりガラスのノイズ | 0.04 |

- **タイリングは .mat 側**（`_BaseMap_ST`。ビルダーは触らない）: ParkBase ×16（20m床にパンチ穴を5cmピッチで）／DrainStation ×6／
  LiftGuide ×4／HighLane・JPRail・JPTube ×3。ほかは ×1（スピンメッシュの u=周方向にそのまま乗る）。
  初回は ParkBase が ×1 で **直径0.8mの水玉**になった——大物メッシュは必ずタイリングを確認すること。

> 4.2 の「点数の数字を焼き込んだ発光パネル」は見送り（TMPラベルで足りている。数字を焼くと点数変更のたびに再生成が要る）。
