...
public static class ParkBuilder
{
    ...
    static void BuildTowerA_Tier23(Transform root)
    {
        ...
        for (int i = 0; i < 4; i++)
        {
            ...
            // r1.80 だとデッキの排出溝が尽きる r1.70 から0.10離れており、溝を出て散った球が
            // ゲート枠の脇を通っていた（User実測映像 2026-08-23）。枠を溝の出口へ寄せる。
            // 4門とも同じ経路で作るので10点ゲートにも同時に効く
            var pos = new Vector3(1.72f * Mathf.Cos(gaz), 4.90f + LiftY, 1.72f * Mathf.Sin(gaz));
            ...
            // 0.40角でもまだ取りこぼす（User報告 2026-08-23）。ゲート枠は y6.06〜6.63 の高さがあり、
            // 弾んで枠の上を越える球は 0.42高（〜6.50）では拾えない。枠全体を覆う 0.60高へ。
            ScoreGateAt(g, pos, gazDeg, gpts[i], new Color(1.0f, 0.45f, 0.35f),
                new Vector3(0.50f, 0.60f, 0.50f));
        }
    }
